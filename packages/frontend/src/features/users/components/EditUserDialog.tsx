...
import { User, Role, Area } from '../types'; // Importar desde ../types/index.ts (Forzando re-evaluación)
        ...
        {userToEdit && (
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              {/* Usar 'none' como valor para "Sin área" y manejarlo en onValueChange */}
              <Select
                value={formData.area_id === null ? 'none' : formData.area_id?.toString() || ''}
                onValueChange={(value) => handleSelectChange('area_id', value === 'none' ? null : (value ? parseInt(value) : null))}
              >
                <SelectTrigger className="col-span-4"><SelectValue placeholder="Seleccionar área" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="none">Sin área</SelectItem> {/* Cambiado value a "none" */}
                  {areas.map((area) => (
