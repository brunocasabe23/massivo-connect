-- Script para crear el permiso gestionar_usuarios
...
BEGIN
    ...
    BEGIN
        ...
        SELECT EXISTS(SELECT 1 FROM permisos WHERE clave = 'gestionar_usuarios') INTO permiso_existe;
        
        IF permiso_existe THEN
            RAISE NOTICE 'El permiso gestionar_usuarios ya existe.';
            
            -- Obtener el ID del permiso existente
            SELECT id INTO permiso_id FROM permisos WHERE clave = 'gestionar_usuarios';
        ELSE
            -- Crear el permiso
            INSERT INTO permisos (clave, descripcion) 
            VALUES ('gestionar_usuarios', 'Permite gestionar usuarios (activar/desactivar)')
            RETURNING id INTO permiso_id;
            
            RAISE NOTICE 'Permiso gestionar_usuarios creado con ID: %', permiso_id;
        END IF;
